...
const Excitement= () => {
    ...
    const [excite, setExcite] = useState([])
    ...
    console.log(excite)
    let excitement_stuff = excite.map((excited, index)=> {
      return    <a href={excited.url} key={index} target="_blank"><div className="w-72 h-72 ">
      <div className="flex h-60 w-60">
        <img src={excited.url} alt="emotions" className="w-full object-cover h-auto rounded-3xl"/>
      </div>
      </div>
      </a>
    })
    return (
        ...
        <h1 className="text-7xl text-center heading">Nigerian Night Life</h1>
