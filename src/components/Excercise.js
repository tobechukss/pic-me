...
const Excercise= () => {
        ...
        function getting() {
            ...
            .then((snapshot) => {
                ...
                snapshot.docs.forEach((doc)=> {
                    ...
                    if (doc.data().formData.activities) {
    ...
    let excercise_stuff = excercise.map((excercising, index)=> {
      return    <a href={excercising.url} key={index} target="_blank"><div className="w-72 h-72 ">
      <div className="flex h-60 w-60">
        <img src={excercising.url} alt="emotions" className="w-full object-cover h-auto rounded-3xl"/>
      </div>
      </div>
      </a>
    })
    return (
        ...
        <h1 className="text-7xl text-center heading">Nigerian Activities</h1>
